...
double fedorets_i_simpson_omp::getSimpson(std::function<double(std::vector<double>)>* f,
...
double getOMPSimpson(std::function<double(std::vector<double>)>* f, std::vector<std::pair<double, double>>* limits,
                     std::vector<int>* n) {
  int dim = n->size();
  std::vector<double> h(dim);
  int64_t counter = 1;
...
bool fedorets_i_simpson_omp::SimpsonSequential::pre_processing() {
  internal_order_test();
...
bool fedorets_i_simpson_omp::SimpsonSequential::validation() {
  internal_order_test();
...
bool fedorets_i_simpson_omp::SimpsonSequential::run() {
  internal_order_test();
  res = getSimpson(f, limits, n);
  return true;
}

bool fedorets_i_simpson_omp::SimpsonSequential::post_processing() {
  internal_order_test();
  reinterpret_cast<double*>(taskData->outputs[0])[0] = res;
  return true;
}

bool fedorets_i_simpson_omp::SimpsonParallel::pre_processing() {
  internal_order_test();
  // Init value for input and output
  // input_ = reinterpret_cast<int*>(taskData->inputs[0])[0];
  f = reinterpret_cast<std::function<double(std::vector<double>)>*>(taskData->inputs[0]);
...
bool fedorets_i_simpson_omp::SimpsonParallel::validation() {
  internal_order_test();
  // Check count elements of output
  // return taskData->inputs_count[0] == 1 && taskData->outputs_count[0] == 1;
  return taskData->inputs_count.size() == 3 && taskData->outputs_count.size() == 1;
...
bool fedorets_i_simpson_omp::SimpsonParallel::run() {
  internal_order_test();
  res = getOMPSimpson(f, limits, n);
  return true;
}

bool fedorets_i_simpson_omp::SimpsonParallel::post_processing() {
  internal_order_test();
  reinterpret_cast<double*>(taskData->outputs[0])[0] = res;
  return true;
}
